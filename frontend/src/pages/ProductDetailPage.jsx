    ...
    <div className="min-h-screen bg-[#FAF9FF] py-8 sm:py-10 lg:py-8">
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 lg:gap-10 xl:gap-12">
          {/* Left Column: Multi-Image High-Res Gallery (~54% desktop width, max ~600px) */}
          <div className="w-full lg:w-[54%] lg:max-w-[600px] shrink-0">
            <ProductGallery images={product.images} name={product.name} />
          </div>

          {/* Right Column: Information & Checkout Actions (~46% desktop width, max ~530px) */}
          <div className="w-full lg:w-[46%] lg:max-w-[530px] shrink-0">
            ...
            {/* ============================================================ */}
            {/* 2. DESKTOP VIEW (lg:): COMPACT, LUXURY & EFFICIENT LAYOUT   */}
            {/* ============================================================ */}
            <div className="hidden lg:block w-full max-w-[530px] space-y-3.5">
              <div>
                {/* Category & Tag */}
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {product.isBestseller && (
                    <span className="px-2 py-0.5 rounded-full text-[9.5px] font-bold uppercase tracking-wider bg-[#17151F] text-[#E8E3FF]">
                      Bestseller
                    </span>
                  )}
                </div>

                {/* Title - Proportional 32px serif typography */}
                <h1 className="font-serif text-[28px] lg:text-[32px] font-normal text-[#17151F] leading-tight tracking-tight">
                  {product.name}
                </h1>
              ...
              {/* Pricing Section */}
              <div className="p-3 xl:p-3.5 rounded-xl bg-white border border-[#D6CFFF]/40 shadow-2xs space-y-0.5">
                {(() => {
                  const effectiveDiscount = (product.discount && product.discount > 0)
                    ? product.discount
                    : (product.originalPrice && product.originalPrice > product.price)
                  ...
                  return (
                    <div className="flex items-baseline gap-2.5 flex-wrap">
                      <span className="font-price text-2xl xl:text-3xl font-bold text-[#17151F] tracking-tight">
                        ₹{product.price.toLocaleString('en-IN')}
                      </span>
                      {product.originalPrice > product.price && (
                        <span className="font-price text-sm xl:text-base text-gray-400 line-through font-normal">
                          ₹{product.originalPrice.toLocaleString('en-IN')}
                        </span>
                      )}
                      {effectiveDiscount > 0 && (
                        <span className="px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-rose-50 text-rose-600 border border-rose-200 shadow-2xs">
                          {effectiveDiscount}% OFF
                        </span>
                      )}
                    </div>
                ...
                <p className="text-[10.5px] text-gray-500 font-light">
              ...
              {product.sizes && product.sizes.length > 0 && (
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="text-[#7464B8] font-semibold text-[11px]">Standard Indian Sizing</span>
                  </div>
                    ...
                    {product.sizes.map((s) => (
                      <button
                        ...
                        className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                          selectedSize === s
                            ? 'border-[#17151F] bg-[#17151F] text-white shadow-xs'
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2.5 bg-white border border-[#D6CFFF]/60 px-2.5 py-1 rounded-xl shadow-2xs">
                  <button
                    onClick={() => setQuantity((q) => Math.max(1, q - 1))}
              ...
              <div className="space-y-2 pt-1">
                <div className="flex gap-2.5">
                  <button
                    onClick={handleAddToCart}
                    className={`flex-1 py-3 xl:py-3.5 text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-md flex items-center justify-center gap-2 btn-shine transition-all ${
                      addedSuccess ? 'bg-emerald-700' : 'bg-[#17151F] hover:bg-[#2A2635]'
                    }`}
                  >
                    {addedSuccess ? (
                  ...
                  <button
                    onClick={() => toggleWishlist(product)}
                    aria-label="Wishlist"
                    className={`w-11 h-11 xl:w-12 xl:h-12 rounded-xl border flex items-center justify-center shadow-2xs transition-all shrink-0 ${
                      inWishlist
                        ? 'bg-rose-50 border-rose-200 text-rose-500'
                        : 'bg-white border-[#D6CFFF]/60 text-gray-700 hover:border-black'
                    }`}
                ...
                <button
                  onClick={handleBuyNow}
                  className="w-full py-2.5 xl:py-3 bg-gradient-to-r from-[#7464B8] to-[#17151F] text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-md hover:opacity-95 transition-opacity flex items-center justify-center gap-2"
                >
                  <Zap className="w-3.5 h-3.5 text-[#D6CFFF]" />
                  <span>Buy Now with Express Checkout</span>
                </button>
              </div>
              ...
              {/* Information Accordions */}
              <div className="space-y-1.5 pt-2 border-t border-[#D6CFFF]/30">
                {accordions.map((acc) => {
                  const isOpen = activeAccordion === acc.id;
                  return (
                    <div key={acc.id} className="rounded-xl bg-white border border-[#D6CFFF]/40 overflow-hidden shadow-2xs">
                      <button
                        onClick={() => setActiveAccordion(isOpen ? '' : acc.id)}
                        className="w-full px-3.5 py-2.5 flex items-center justify-between text-left text-xs font-bold uppercase tracking-wider text-gray-900 hover:text-[#7464B8] transition-colors"
                      >
                        <span>{acc.title}</span>
                        <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
                      </button>
                      <AnimatePresence>
                        {isOpen && (
                          <motion.div
                            ...
                            exit={{ height: 0, opacity: 0 }}
                            transition={{ duration: 0.2 }}
                            className="px-3.5 pb-3 pt-1 border-t border-gray-100 text-xs text-gray-600 leading-relaxed font-light"
                          >
                            {acc.content}
                          </motion.div>
                        )}
        ...
        <div id="reviews-section" className="mt-10 sm:mt-12 lg:mt-10 pt-6 sm:pt-8 lg:pt-6 border-t border-[#D6CFFF]/40">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 lg:mb-4">
            <div>
              <span className="text-[11px] lg:text-xs font-bold uppercase tracking-widest text-[#7464B8]">Patron Feedback</span>
              <h3 className="font-serif text-xl sm:text-2xl lg:text-2xl font-light text-[#17151F] mt-0.5">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 sm:px-5 sm:py-2.5 bg-[#17151F] text-white rounded-xl text-xs font-semibold uppercase tracking-wider hover:bg-[#2A2635] shadow-sm btn-shine"
            >
              <MessageSquarePlus className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#D6CFFF]" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
            ...
            {product.testimonial && product.testimonial.reviewText && (
              <div className="p-5 lg:p-4 xl:p-5 rounded-2xl bg-gradient-to-br from-white via-[#FCFBFF] to-[#F7F4FF] border border-[#7464B8]/40 shadow-xs space-y-2 lg:space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9.5px] font-semibold text-[#7464B8] uppercase tracking-wider bg-[#F3EFFF] px-2.5 py-0.5 rounded-full border border-[#D6CFFF]/60">
                ...
                <div className="pt-2.5 border-t border-[#EDE7FA] flex items-center justify-between text-[11px]">
            ...
            {reviews.length > 0 ? (
              reviews.map((rev) => (
                <div key={rev._id} className="p-5 lg:p-4 xl:p-5 rounded-2xl bg-white border border-[#D6CFFF]/40 shadow-xs space-y-2 lg:space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 text-amber-500">
              ...
              <div className="col-span-2 text-center py-8 lg:py-10 bg-white rounded-2xl border border-[#D6CFFF]/40 p-5">
