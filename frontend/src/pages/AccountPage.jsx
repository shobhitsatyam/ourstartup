...
export default function AccountPage({ initialAuthMode }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9FF] py-5 sm:py-8 lg:py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6 lg:space-y-8">
        {/* User Welcome Banner Card */}
        <div className="p-4 sm:p-5 lg:p-8 rounded-2xl sm:rounded-3xl bg-gradient-to-r from-[#17151F] via-[#2A2635] to-[#17151F] text-white border border-[#D6CFFF]/30 shadow-lg sm:shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 sm:gap-4 lg:gap-6 relative overflow-hidden">
          <div className="flex items-start justify-between sm:block min-w-0">
            <div className="min-w-0 pr-2 sm:pr-0">
              <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-[#D6CFFF] block">
                Zivana Jewels Patron
              </span>
              <h1 className="font-serif text-lg sm:text-2xl lg:text-3xl font-light mt-0.5 sm:mt-1 truncate">
                Namaste, {user.name}
              </h1>
              <p className="text-[11px] sm:text-xs text-white/60 font-light mt-0.5 truncate">
                {user.email}
              </p>
            </div>

            {/* Mobile-only Logout & Admin shortcut */}
            <div className="flex sm:hidden items-center gap-1.5 shrink-0 pt-0.5">
              {isAdmin && (
                <Link
                  to="/admin"
                  className="px-2.5 py-1.5 rounded-xl bg-[#D6CFFF] text-[#17151F] font-bold text-[10px] uppercase tracking-wider flex items-center gap-1 shadow"
                >
              ...
              <button
                onClick={logout}
                className="p-2 rounded-xl bg-white/10 text-white/80 hover:text-rose-400 hover:bg-white/20 transition-colors"
                title="Sign Out"
                aria-label="Sign Out"
              >
          ...
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
            ...
            <div
              ...
              className="w-full sm:w-auto px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md border border-[#D6CFFF]/40 flex items-center justify-between sm:justify-start gap-2.5 cursor-pointer hover:bg-white/20 transition-colors shadow-sm sm:shadow-md"
            >
              ...
              <span className="text-xs font-bold text-[#D6CFFF]">
                {user.oceanPoints || 0} Zivana Points (₹{user.oceanPoints || 0})
              </span>
            </div>

            {/* Admin Portal Link if Admin (Tablet/Desktop) */}
            {isAdmin && (
              <Link
                to="/admin"
                className="hidden sm:flex px-4 py-2.5 rounded-2xl bg-[#D6CFFF] text-[#17151F] font-bold text-xs uppercase tracking-wider items-center gap-2 shadow-lg hover:opacity-95 transition-opacity"
              >
            ...
            <button
              onClick={logout}
              className="hidden sm:block p-2.5 rounded-2xl bg-white/10 text-white/80 hover:text-rose-400 hover:bg-white/20 transition-colors"
              title="Sign Out"
              aria-label="Sign Out"
            >
